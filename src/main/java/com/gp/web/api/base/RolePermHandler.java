...
public class RolePermHandler extends BaseApiSupport{
	...
	public void handleRoleAdd(HttpServerExchange exchange)throws BaseException{
		...
		role.setInfoId(IdKeys.getInfoId(NodeIdKey.ROLE, Filters.filterLong(params, "role_id")));
		...
		Set<ValidateMessage> vmsg = ValidateUtils.validate(principal.getLocale(), role);
		RoleInfo role1 = rolePermSvc.getRole(Filters.filterString(params, "role_abbr"));
		if(null != role1 && !role1.getId().equals(role.getId())) {
			vmsg.add(new ValidateMessage("role_abbr", getMessage(exchange, "mesg.role.existed")));
		}
	...
	public void handleRoleUpdate(HttpServerExchange exchange)throws BaseException{
		...
		// check the validation of user information
		Set<ValidateMessage> vmsg = ValidateUtils.validate(svcctx.getPrincipal().getLocale(), role);
		RoleInfo role1 = rolePermSvc.getRole(Filters.filterString(params, "role_abbr"));
		if(null != role1 && !role1.getId().equals(role.getId())) {
			vmsg.add(new ValidateMessage("role_abbr", getMessage(exchange, "mesg.role.existed")));
		}
	...
	public void handleRoleEndpointsQuery(HttpServerExchange exchange)throws BaseException{
		...
		InfoId rId = IdKeys.getInfoId(NodeIdKey.ROLE, roleId);
	...
	public void handlePermissionGrant(HttpServerExchange exchange)throws BaseException{
		...
		boolean success ;
		InfoId rId = IdKeys.getInfoId(NodeIdKey.ROLE, roleId);
		InfoId endId = IdKeys.getInfoId(NodeIdKey.ENDPOINT, endpointId);
		if(authorized) {
	...
	public void handleRoleMemberAdd(HttpServerExchange exchange) throws BaseException{
		...
		InfoId roleId = IdKeys.getInfoId(NodeIdKey.ROLE, NumberUtils.toLong(idStr));
