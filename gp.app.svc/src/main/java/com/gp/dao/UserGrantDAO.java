...
import com.gp.dao.info.UserGrantInfo;

@BindComponent(type = UserGrantDAO.class, priority = BaseDAO.BASE_PRIORITY)
public class UserGrantDAO extends DAOSupport implements BaseDAO<UserGrantInfo> {

    private Identifier daoIdKey = BaseIdKey.BLIND;

	static Logger LOGGER = LoggerFactory.getLogger(UserGrantDAO.class);

	static final String[] COLUMNS = new String[]{
             "user_id", "org_id", "dept_id", "duty_id", 
	...
	public static final RowParser<UserGrantInfo> INFO_PARSER = (rp, ctx) -> {
		...
		info.setDeptId(rp.getLong("dept_id"));
	...
	public static final RowMapper<UserGrantInfo> INFO_MAPPER = (rs, ctx) -> {
		...
		info.setDeptId(rs.getLong("dept_id"));
    ...
    @Override
	public RowParser<UserGrantInfo> getRowParser(){
    	return INFO_PARSER;
    }

    @Override
	public RowMapper<UserGrantInfo> getRowMapper(){
		if(isCrossDB()){
			return new ParserBuilder<UserGrantInfo>().build(INFO_PARSER);
		}
		return INFO_MAPPER;
    }
	 ...
	 **/
	public Object[] getValues(UserGrantInfo info, String ...columns){
		BeanMeta meta = BeanAccessor.getBeanMeta(UserGrantInfo.class);
	...
	@Override
	public int create(UserGrantInfo info) {
	...
	@Override
	public int update(UserGrantInfo info) {
	...
	@Override
	public UserGrantInfo row( InfoId id ){
