...
@BindComponent(type = EndpointDAO.class, priority = BaseDAO.BASE_PRIORITY)
public class EndpointDAO extends DAOSupport implements BaseDAO<EndpointInfo> {

    private Identifier daoIdKey = BaseIdKey.BLIND;

	static Logger LOGGER = LoggerFactory.getLogger(EndpointDAO.class);

	static final String[] COLUMNS = new String[]{
             "endpoint_name", "module", "type", "endpoint_abbr", 
             "access_path", "description", "modifier_uid", "modify_time", 
             "del_flag"
    	};
	...
	public static final RowParser<EndpointInfo> INFO_PARSER = (rp, ctx) -> {
		...
		info.setModule(rp.getString("module"));
		info.setType(rp.getString("type"));
		info.setEndpointAbbr(rp.getString("endpoint_abbr"));
    ...
    @Override
	public RowParser<EndpointInfo> getRowParser(){
    	return INFO_PARSER;
    }

    @Override
	public RowMapper<EndpointInfo> getRowMapper(){
		if(isCrossDB()){
			return new ParserBuilder<EndpointInfo>().build(INFO_PARSER);
		}
		return INFO_MAPPER;
    }
	 ...
	 **/
	public Object[] getValues(EndpointInfo info, String ...columns){
		BeanMeta meta = BeanAccessor.getBeanMeta(EndpointInfo.class);
	...
	@Override
	public int create(EndpointInfo info) {
	...
	@Override
	public int update(EndpointInfo info) {
	...
	@Override
	public EndpointInfo row( InfoId id ){
