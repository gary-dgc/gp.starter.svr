...
@BindComponent(type = DataModelDAO.class, priority = BaseDAO.BASE_PRIORITY)
public class DataModelDAO extends DAOSupport implements BaseDAO<DataModelInfo> {

    private Identifier daoIdKey = BaseIdKey.BLIND;

	static Logger LOGGER = LoggerFactory.getLogger(DataModelDAO.class);

	static final String[] COLUMNS = new String[]{
             "sys_id", "data_name", "description", "modifier_uid", 
	...
	public static final RowParser<DataModelInfo> INFO_PARSER = (rp, ctx) -> {
		...
		info.setDataName(rp.getString("data_name"));
	...
	public static final RowMapper<DataModelInfo> INFO_MAPPER = (rs, ctx) -> {
		...
		info.setDataName(rs.getString("data_name"));
    ...
    @Override
	public RowParser<DataModelInfo> getRowParser(){
    	return INFO_PARSER;
    }

    @Override
	public RowMapper<DataModelInfo> getRowMapper(){
		if(isCrossDB()){
			return new ParserBuilder<DataModelInfo>().build(INFO_PARSER);
		}
		return INFO_MAPPER;
    }
	 ...
	 **/
	public Object[] getValues(DataModelInfo info, String ...columns){
		BeanMeta meta = BeanAccessor.getBeanMeta(DataModelInfo.class);
	...
	@Override
	public int create(DataModelInfo info) {
	...
	@Override
	public int update(DataModelInfo info) {
	...
	@Override
	public DataModelInfo row( InfoId id ){
