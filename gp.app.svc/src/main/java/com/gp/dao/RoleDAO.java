...
@BindComponent(type = RoleDAO.class, priority = BaseDAO.BASE_PRIORITY)
public class RoleDAO extends DAOSupport implements BaseDAO<RoleInfo> {

    private Identifier daoIdKey = BaseIdKey.BLIND;

	static Logger LOGGER = LoggerFactory.getLogger(RoleDAO.class);

	static final String[] COLUMNS = new String[]{
             "sys_id", "role_name", "role_abbr", "reserved", 
             "description", "default_case", "modifier_uid", "modify_time", 
             "del_flag"
    	};

	public static final RowParser<RoleInfo> INFO_PARSER = (rp, ctx) -> {
		RoleInfo info = new RoleInfo();
		InfoId id = rp.getInfoId(MasterIdKey.ROLE);
		info.setInfoId(id);

		info.setSysId(rp.getLong("sys_id"));
		info.setRoleName(rp.getString("role_name"));
		info.setRoleAbbr(rp.getString("role_abbr"));
		info.setReserved(rp.getBoolean("reserved"));
		info.setDescription(rp.getString("description"));
		info.setDefaultCase(rp.getString("default_case"));
	...
	public static final RowMapper<RoleInfo> INFO_MAPPER = (rs, ctx) -> {
		RoleInfo info = new RoleInfo();
		InfoId id = IdKeys.getInfoId(MasterIdKey.ROLE, rs.getLong("role_id"));
		info.setInfoId(id);

		info.setSysId(rs.getLong("sys_id"));
		info.setRoleName(rs.getString("role_name"));
		info.setRoleAbbr(rs.getString("role_abbr"));
		info.setReserved(rs.getBoolean("reserved"));
		info.setDescription(rs.getString("description"));
		info.setDefaultCase(rs.getString("default_case"));
	 ...
	 **/
	public RoleDAO(){
		this.daoIdKey = MasterIdKey.ROLE;
	}
    ...
    @Override
	public RowParser<RoleInfo> getRowParser(){
    	return INFO_PARSER;
    }
	...
	public RowMapper<RoleInfo> getRowMapper(){
		if(isCrossDB()){
			return new ParserBuilder<RoleInfo>().build(INFO_PARSER);
		}
		return INFO_MAPPER;
    }
	 ...
	 **/
	public Object[] getValues(RoleInfo info, String ...columns){
		BeanMeta meta = BeanAccessor.getBeanMeta(RoleInfo.class);
