...
@BindComponent(type = ClientDAO.class, priority = BaseDAO.BASE_PRIORITY)
public class ClientDAO extends DAOSupport implements BaseDAO<ClientInfo> {

    private Identifier daoIdKey = BaseIdKey.BLIND;

	static Logger LOGGER = LoggerFactory.getLogger(ClientDAO.class);

	static final String[] COLUMNS = new String[]{
             "client_name", "scope", "client_key", "client_secret", 
             "claims_json", "modifier_uid", "modify_time", "del_flag", 
    	};

	public static final RowParser<ClientInfo> INFO_PARSER = (rp, ctx) -> {
		ClientInfo info = new ClientInfo();
		InfoId id = rp.getInfoId(BaseIdKey.CLIENT);
		info.setInfoId(id);

		info.setClientName(rp.getString("client_name"));
		info.setScope(rp.getString("scope"));
		info.setClientKey(rp.getString("client_key"));
		info.setClientSecret(rp.getString("client_secret"));
		info.setClaimsJson(rp.getString("claims_json"));
	...
	public static final RowMapper<ClientInfo> INFO_MAPPER = (rs, ctx) -> {
		ClientInfo info = new ClientInfo();
		InfoId id = IdKeys.getInfoId(BaseIdKey.CLIENT, rs.getLong("client_id"));
		info.setInfoId(id);

		info.setClientName(rs.getString("client_name"));
		info.setScope(rs.getString("scope"));
		info.setClientKey(rs.getString("client_key"));
		info.setClientSecret(rs.getString("client_secret"));
		info.setClaimsJson(rs.getString("claims_json"));
	 ...
	 **/
	public ClientDAO(){
		this.daoIdKey = BaseIdKey.CLIENT;
	}
    ...
    @Override
	public RowParser<ClientInfo> getRowParser(){
    	return INFO_PARSER;
    }

    @Override
	public RowMapper<ClientInfo> getRowMapper(){
		if(isCrossDB()){
			return new ParserBuilder<ClientInfo>().build(INFO_PARSER);
		}
		return INFO_MAPPER;
    }
	 ...
	 **/
	public Object[] getValues(ClientInfo info, String ...columns){
		BeanMeta meta = BeanAccessor.getBeanMeta(ClientInfo.class);
	...
	@Override
	public int create(ClientInfo info) {
	...
	@Override
	public int update(ClientInfo info) {
	...
	@Override
	public ClientInfo row( InfoId id ){
