...
import com.gp.dao.info.DutyFlatInfo;

@BindComponent(type = DutyFlatDAO.class, priority = BaseDAO.BASE_PRIORITY)
public class DutyFlatDAO extends DAOSupport implements BaseDAO<DutyFlatInfo> {

    private Identifier daoIdKey = BaseIdKey.BLIND;

	static Logger LOGGER = LoggerFactory.getLogger(DutyFlatDAO.class);

	static final String[] COLUMNS = new String[]{
             "duty_pid", "duty_leaf_id", "hier_lvl", "modifier_uid", 
	...
	public static final RowParser<DutyFlatInfo> INFO_PARSER = (rp, ctx) -> {
		...
		info.setDutyLeafId(rp.getLong("duty_leaf_id"));
	...
	public static final RowMapper<DutyFlatInfo> INFO_MAPPER = (rs, ctx) -> {
		...
		info.setDutyLeafId(rs.getLong("duty_leaf_id"));
    ...
    @Override
	public RowParser<DutyFlatInfo> getRowParser(){
    	return INFO_PARSER;
    }

    @Override
	public RowMapper<DutyFlatInfo> getRowMapper(){
		if(isCrossDB()){
			return new ParserBuilder<DutyFlatInfo>().build(INFO_PARSER);
		}
		return INFO_MAPPER;
    }
	 ...
	 **/
	public Object[] getValues(DutyFlatInfo info, String ...columns){
		BeanMeta meta = BeanAccessor.getBeanMeta(DutyFlatInfo.class);
	...
	@Override
	public int create(DutyFlatInfo info) {
	...
	@Override
	public int update(DutyFlatInfo info) {
	...
	@Override
	public DutyFlatInfo row( InfoId id ){
